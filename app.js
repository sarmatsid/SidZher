...
const { stringify, unescape } = require('querystring'); // требуется для проверки captcha для приведения в строковый формат передаваемых пареметров
...
app.post('/api/register_step1', (req, res) => { // получаю post-запрос от login.js со стороны пользователя
   ...
   let json = {
      ...
      "user": unescape(req.body["Login"]), // логин
